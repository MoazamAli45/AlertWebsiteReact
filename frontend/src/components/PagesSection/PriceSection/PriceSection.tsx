...
const PriceSection: React.FC = () => {
  return (
    <Wrapper>
      {' '}
      <section className="flex flex-col  gap-[4rem] mb-[70px]" id="price">
          ...
          {data.map((item, id) => (
            <div className="col-span-1" key={id}>
              <div className="flex justify-center -mb-[27px] mr-[7px]">
                <button className="btn-price py-[10px]  text-[14px]  ">
                  {item.btn}
                </button>
              </div>
              <div className="w-full h-[480px]  rounded-3xl relative flex flex-col py-[40px] px-[20px] gap-[30px]">
                <img
                  src={PriceCard}
                  alt="Price Card"
                ...
                <div className="flex justify-between items-center">
                  <div className="flex  items-center">
                    <DiamondIcon />
                    <h4 className="text-[18px] text-white text-bold">
                      {item.type}
                    </h4>
                  </div>
                  <div className="flex flex-col">
                    <h3 className="text-[26px] font-bold">{item.price}</h3>
                    {item.save && (
                      <span className="text-[14px] font-normal">Save 20%</span>
                    )}
                  </div>
                </div>
                <div
                ...
                {item.showBtn && (
                  <div className="w-[90%] mx-auto">
                    <button className="btn-login flex items-center w-full justify-center py-[10px] rounded-full">
                      Subscribe
                    </button>
                  </div>
                )}
